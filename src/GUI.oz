...
%%%%% Squares of water and island
	Squares = square(0:label(image:{QTk.newImage photo(file:'img/water.gif')} height:1 width:1)
 	      1:label(image:{QTk.newImage photo(file:'img/island.gif')} height:1 width:1)
 	     )

%%%%% Labels for rows and columns
	fun{Label V}
		label(text:V relief:groove borderwidth:5 bg:c(0 47 167) ipadx:5 ipady:5 foreground:white)
	end
...
%%%%% Init the submarine
	...
	in
		...
		LabelSub = label(image:{QTk.newImage photo(file:'img/'#Color#'Sub.gif')} handle:Handle height:1 width:1)
		if(Color == black) then
				LabelScore = label(text:Input.maxDamage borderwidth:5 handle:HandleScore relief:groove bg:Color ipadx:5 ipady:5 foreground:white)
		else
				LabelScore = label(text:Input.maxDamage borderwidth:5 handle:HandleScore relief:groove bg:Color ipadx:5 ipady:5)
		end
	...
	fun{DrawMine Position}
		fun{$ Grid State}
			...
			guiPlayer(id:ID score:HandleScore submarine:Handle mines:Mine path:Path) = State
			pt(x:X y:Y) = Position
			LabelMine = label(image:{QTk.newImage photo(file:'img/mine.gif')} handle:HandleMine height:1 width:1)
			{Grid.grid configure(LabelMine row:X+1 column:Y+1 sticky:wesn)}
			{HandleMine 'raise'()}
			{Handle 'raise'()}
	...
	fun{DrawPath Grid Color X Y}
		Handle LabelPath
	in
		LabelPath = label(image:{QTk.newImage photo(file:'img/'#Color#'.gif')} handle:Handle height:1 width:1)
		{Grid.grid configure(LabelPath row:X+1 column:Y+1 sticky:wesn)}
		Handle
	end
